...
| `packages/plugin/src/index.ts`                              | Plugin entry — composes hooks (event, tool, command), structured logging, toast notifications, spawns overlay. `/pet` command handoff to TUI plugin via `client.tui.publish()`                          |
...
| `packages/plugin/src/tui/index.tsx`                         | TUI plugin — registers `pet.show_dialog_launch`/`pet.show_dialog_toggle` keymap commands that show a DialogAlert (3s auto-close, Esc/Enter dismiss) to overlay the sentinel error from the `/pet` throw |
